...
module.exports = {
  postPrayerReport: async (req, res) => {
    ...
    try {
      ...
      let refWeekDay = refWeekFromMonfunction.refWeekFromMonfunction(
        day.getTime()
      );
      ...
      prayerReports.forEach(e => {
        ...
        arrPrayerReports.push(
          refWeekFromMonfunction.refWeekFromMonfunction(elem.getTime())
        );
      });
  ...
  getPrayerReports: async (req, res) => {
    try {
      ...
      for (let i = prayerReports.length - 1; i >= 0; i--) {
        ...
        let id = prayerReports[i]._id;
        let datePosted = prayerReports[i].date;
        ...
        if (datePrayed.getFullYear() === new Date().getFullYear()) {
          ...
          let weekNumAndDatePrayed = [
            findThisWeekNumber.findThisWeekNumber(mslastMon),
            datePrayed,
            refWeek,
            id,
            datePosted
  ...
  deletePrayerReport: async (req, res) => {
    try {
        await Prayer.findOneAndRemove({_id: req.params.id});
